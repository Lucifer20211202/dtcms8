		...
		<dt-toolbar-box>
			<template #default>
				<div class="list-box">
					<div class="l-list">
						<el-button-group>
							<el-button icon="Plus" @click="$common.linkUrl('/member/recharge/edit')">新增</el-button>
							<el-button icon="Finished" @click="completeCheckAll">完成</el-button>
		...
		<div class="content-box">
			<el-card class="table-card">
				<el-table ref="tableRef" v-loading="datas.loading" :data="datas.listData" stripe class="table-list" @selection-change="handleSelectionChange">
					...
					<el-table-column prop="paymentTitle" label="支付方式" min-width="100"></el-table-column>
					<el-table-column prop="amount" label="金额" width="100"></el-table-column>
					<el-table-column prop="status" label="状态" width="100">
						<template #default="scope">
							<el-tag size="small" type="warning" effect="dark" v-if="scope.row.status==1">已完成</el-tag>
							<el-tag size="small" type="success" effect="dark" v-else-if="scope.row.status==2">已取消</el-tag>
							<el-tag size="small" type="info" effect="dark" v-else>待完成</el-tag>
						</template>
					</el-table-column>
					<el-table-column prop="addTime" label="充值时间" width="160"></el-table-column>
					<el-table-column fixed="right" label="操作" width="90">
						<template #default="scope">
							<el-button size="small" icon="View" @click="$common.linkUrl(`/member/recharge/edit/${scope.row.id}`)"></el-button>
...
<script setup>
	...
	const initData = async() => {
		let sendUrl = `/admin/member/recharge?pageSize=${datas.pageSize}&pageIndex=${datas.pageIndex}`
	...
	const deleteCheckAll = () => {
		...
		proxy.$confirm('确认要删除该记录吗？', '提示', {
			...
			proxy.$api.request({
				method: 'delete',
				url: `/admin/member/recharge?ids=${ids.toString()}`,
				loading: true,
				successMsg: '已删除成功',
				success(res) {
	...
	//完成充值
	const completeCheckAll = () => {
		//拿到选中的数据
		let list = datas.multipleSelection
		//检查是否有选中
		if (!list.length) {
			proxy.$message({ type: 'warning', message: '请选择要完成的记录' })
			return false
		}
		//执行完成操作
		proxy.$confirm('此操作会员余额增加，确认继续吗？', '提示', {
			confirmButtonText: '确定',
			cancelButtonText: '取消',
			type: 'warning'
		}).then(() => {
