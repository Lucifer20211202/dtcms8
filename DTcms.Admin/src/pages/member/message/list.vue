<template>
	<div class="mainbody">
		<dt-location :data="[{title:'会员日志'},{title:'消息列表'}]"></dt-location>
		<dt-toolbar-box>
			<template #default>
				<div class="list-box">
					<div class="l-list">
						<el-button-group>
							<el-button icon="Plus" @click="$common.linkUrl('/member/message/edit')">新增</el-button>
		...
		<div class="content-box">
			<el-card class="table-card">
				<el-table ref="tableRef" v-loading="datas.loading" :data="datas.listData" stripe class="table-list" @selection-change="handleSelectionChange">
					...
					<el-table-column prop="title" label="消息标题" min-width="150"></el-table-column>
					<el-table-column prop="isRead" label="状态" width="100">
						<template #default="scope">
							<el-tag size="small" type="warning" effect="dark" v-if="scope.row.isRead==0">未读</el-tag>
							<el-tag size="small" type="info" effect="dark" v-else>已读</el-tag>
						</template>
					</el-table-column>
					<el-table-column prop="addTime" label="发送时间" width="160"></el-table-column>
					<el-table-column fixed="right" label="操作" width="90">
						<template #default="scope">
							<el-button size="small" icon="Edit" @click="$common.linkUrl(`/member/message/edit/${scope.row.id}`)"></el-button>
...
<script setup>
	...
	const initData = async() => {
		let sendUrl = `/admin/member/message?pageSize=${datas.pageSize}&pageIndex=${datas.pageIndex}`
	...
	const deleteCheckAll = () => {
		...
		proxy.$confirm('确认要删除该记录吗？', '提示', {
			...
			proxy.$api.request({
				method: 'delete',
				url: `/admin/member/message?ids=${ids.toString()}`,
				loading: true,
				successMsg: '已删除成功',
				success(res) {
