<template>
	<div class="mainbody">
		<dt-location :data="[{title:'会员日志'},{title:'模板列表'}]"></dt-location>
		<dt-toolbar-box>
			<template #default>
				<div class="list-box">
					<div class="l-list">
						<el-button-group>
							<el-button icon="Plus" @click="$common.linkUrl('/member/template/edit')">新增</el-button>
				...
				<el-table ref="tableRef" v-loading="datas.loading" :data="datas.listData" stripe class="table-list" @selection-change="handleSelectionChange">
					...
					<el-table-column label="模板类型" width="120">
						<template #default="scope">
							<el-tag size="small" effect="dark" v-if="scope.row.type==1">邮件模板</el-tag>
							<el-tag size="small" type="danger" effect="dark" v-else-if="scope.row.type==2">短信模板</el-tag>
							<el-tag size="small" type="success" effect="dark" v-else-if="scope.row.type==3">微信模板</el-tag>
						</template>
					</el-table-column>
					<el-table-column prop="callIndex" label="调用名称" width="120"></el-table-column>
					<el-table-column prop="title" label="标题" min-width="120"></el-table-column>
					<el-table-column fixed="right" label="操作" width="90">
						<template #default="scope">
							<el-button size="small" icon="Edit" @click="$common.linkUrl(`/member/template/edit/${scope.row.id}`)"></el-button>
...
<script setup>
	...
	const initData = async() => {
		let sendUrl = `/admin/notify/template?pageSize=${datas.pageSize}&pageIndex=${datas.pageIndex}`
	...
	const deleteCheckAll = () => {
		...
		proxy.$confirm('确认要删除该记录吗？', '提示', {
			...
			proxy.$api.request({
				method: 'delete',
				url: `/admin/notify/template?ids=${ids.toString()}`,
				loading: true,
				successMsg: '已删除成功',
				success(res) {
