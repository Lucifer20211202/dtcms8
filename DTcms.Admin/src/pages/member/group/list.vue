<template>
	<div class="mainbody">
		<dt-location :data="[{title:'会员管理'},{title:'会员组列表'}]"></dt-location>
		<dt-toolbar-box>
			<template #default>
				<div class="list-box">
					<div class="l-list">
						<el-button-group>
							<el-button icon="Plus" @click="$common.linkUrl('/member/group/edit')">新增</el-button>
		...
		<div class="content-box">
			<el-card class="table-card">
				<el-table ref="tableRef" v-loading="datas.loading" :data="datas.listData" stripe class="table-list" @selection-change="handleSelectionChange">
					...
					<el-table-column prop="amount" label="预存(元)" width="100"></el-table-column>
					<el-table-column prop="amount" label="经验值" width="90"></el-table-column>
					<el-table-column prop="addTime" label="创建时间" width="160"></el-table-column>
					<el-table-column label="可升级" width="90" align="center">
						<template #default="scope">
							<el-icon v-if="scope.row.isUpgrade===1"><Check /></el-icon>
							<el-icon v-else><Minus /></el-icon>
						</template>
					</el-table-column>
					<el-table-column label="默认" width="90" align="center">
						<template #default="scope">
							<el-switch :active-value="1" :inactive-value="0" v-model="scope.row.isDefault"
										@change="updatePatch(scope.row.id, '/isDefault', scope.row.isDefault)"></el-switch>
						</template>
					</el-table-column>
					<el-table-column label="状态" width="90" align="center">
						<template #default="scope">
							<el-switch :active-value="0" :inactive-value="1" v-model="scope.row.status"
										@change="updatePatch(scope.row.id, '/status', scope.row.status)"></el-switch>
						</template>
					</el-table-column>
					<el-table-column fixed="right" label="操作" width="90">
						<template #default="scope">
							<el-button size="small" icon="Edit" @click="$common.linkUrl(`/member/group/edit/${scope.row.id}`)"></el-button>
...
<script setup>
	...
	const initData = async() => {
		let sendUrl = `/admin/member/group?pageSize=${datas.pageSize}&pageIndex=${datas.pageIndex}`
	...
	const deleteCheckAll = () => {
		...
		proxy.$confirm('确认要删除该记录吗？', '提示', {
			...
			proxy.$api.request({
				method: 'delete',
				url: `/admin/member/group?ids=${ids.toString()}`,
				loading: true,
				successMsg: '已删除成功',
				success(res) {
	...
	//修改部分字段
	const updatePatch = (id, path, val) => {
		proxy.$api.request({
			method: 'patch',
			url: `/admin/member/group/${id}`,
