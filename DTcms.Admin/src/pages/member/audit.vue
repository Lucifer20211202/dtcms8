<template>
	<div class="mainbody">
		<dt-location :data="[{title:'会员管理'},{title:'会员审核'}]"></dt-location>
		<dt-toolbar-box>
			<template #default>
				<div class="list-box">
					<div class="l-list">
				...
				<el-table ref="tableRef" v-loading="datas.loading" :data="datas.listData" stripe class="table-list" @selection-change="handleSelectionChange">
					...
					<el-table-column label="头像" width="60">
						<template #default="scope">
							<el-avatar :size="40" :src="$api.loadFile(scope.row.avatar)" v-if="scope.row.avatar"></el-avatar>
							<el-avatar :size="40" icon="el-icon-user-solid" v-else></el-avatar>
						</template>
					</el-table-column>
					<el-table-column label="用户名" min-width="150">
						<template #default="scope">
							<h4>
								{{scope.row.userName}}
								<b v-if="scope.row.realName">({{scope.row.realName}})</b>
							</h4>
							<span class="date">
					...
					<el-table-column prop="amount" label="余额" width="90" align="center"></el-table-column>
					<el-table-column prop="point" label="积分" width="90" align="center"></el-table-column>
					...
					<el-table-column fixed="right" label="操作" width="90">
						<template #default="scope">
							<el-button size="small" type="danger" icon="Delete" @click="deleteItem(scope.row.userId)"></el-button>
						</template>
					</el-table-column>
				</el-table>
			</el-card>
...
<script setup>
	...
	const initData = async() => {
		let sendUrl = `/admin/member?status=2&pageSize=${datas.pageSize}&pageIndex=${datas.pageIndex}`
	...
	const deleteCheckAll = () => {
		...
		proxy.$confirm('确认要删除该记录吗？', '提示', {
			...
			proxy.$api.request({
				method: 'delete',
				url: `/admin/member?ids=${ids.toString()}`,
				loading: true,
				successMsg: '已删除成功',
				success(res) {
	...
	const auditCheckAll = () => {
		...
		//检查是否有选中
		if (!list.length) {
			proxy.$message({ type: 'warning', message: '请选择要审核的会员' })
			return false
		}
		//执行审核操作
		proxy.$confirm('确认要更改会员状态吗？', '提示', {
			confirmButtonText: '确定',
			cancelButtonText: '取消',
			type: 'warning'
		}).then(function () {
