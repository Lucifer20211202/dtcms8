<template>
	<div class="mainbody">
		<dt-location :data="[{title:'文章管理'},{title:'类别列表'}]"></dt-location>
		<dt-toolbar-box>
			<template #default>
				<div class="list-box">
					<div class="l-list">
						<el-button-group>
							<el-button icon="Plus" @click="$common.linkUrl(`/article/category/edit/${props.channelId}`)">新增</el-button>
			...
			<el-card class="table-card">
				<el-table ref="tableRef" v-loading="datas.loading" :data="datas.listData" row-key="id" default-expand-all stripe
					...
					<el-table-column fixed="right" label="操作" width="120">
						<template #default="scope">
							<el-button size="small" icon="Edit" @click="$common.linkUrl(`/article/category/edit/${props.channelId}/${scope.row.id}`)"></el-button>
							<el-button size="small" icon="Plus" @click="$common.linkUrl(`/article/category/edit/${props.channelId}`, {parentId:scope.row.id})"></el-button>
...
<script setup>
	...
	const initData = async() => {
		let sendUrl = `/admin/article/category/${props.channelId}`
	...
	const deleteCheckAll = () => {
		...
		proxy.$confirm('确认要删除该记录吗？', '提示', {
			...
			proxy.$api.request({
				method: 'delete',
				url: `/admin/article/category/${props.channelId}?ids=${ids.toString()}`,
				loading: true,
				successMsg: '已删除成功',
				success(res) {
	...
	const deleteItem = (val) => {
		...
		proxy.$confirm('确认要删除该记录吗？', '提示', {
			...
			proxy.$api.request({
				method: 'delete',
				url: `/admin/article/category/${props.channelId}/${val}`,
				loading: true,
				successMsg: '已删除成功',
				success(res) {
					initData() //重新加载列表
	...
	const updateField = (id, path, val) => {
		proxy.$api.request({
			method: 'patch',
			url: `/admin/article/category/${props.channelId}/${id}`,
