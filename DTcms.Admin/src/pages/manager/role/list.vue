<template>
	<div class="mainbody">
		<dt-location :data="[{title:'系统管理'},{title:'角色列表'}]"></dt-location>
		<dt-toolbar-box>
			<template #default>
				<div class="list-box">
					<div class="l-list">
						<el-button-group>
							<el-button icon="Plus" @click="$common.linkUrl('/manager/role/edit')">新增</el-button>
		...
		<div class="content-box">
			<el-card class="table-card">
				<el-table ref="tableRef" v-loading="datas.loading" :data="datas.listData" stripe class="table-list" @selection-change="handleSelectionChange">
					...
					<el-table-column prop="name" label="角色标识" min-width="120"></el-table-column>
					<el-table-column prop="title" label="角色名称" min-width="150"></el-table-column>
					<el-table-column prop="roleType" label="角色类型" width="120">
						<template #default="scope">
							<el-tag size="small" type="info" effect="dark" v-if="scope.row.roleType==0">普通会员</el-tag>
							<el-tag size="small" type="warning" effect="dark" v-else-if="scope.row.roleType==1">系统管理员</el-tag>
							<el-tag size="small" type="success" effect="dark" v-else-if="scope.row.roleType==2">超级管理员</el-tag>
						</template>
					</el-table-column>
					<el-table-column prop="isSystem" label="系统默认" width="90" align="center">
						<template #default="scope">
							<el-icon v-if="scope.row.isSystem==1"><Check /></el-icon>
							<el-icon v-else><Minus /></el-icon>
						</template>
					</el-table-column>
					<el-table-column fixed="right" label="操作" width="90">
						<template #default="scope">
							<el-button size="small" icon="Edit" @click="$common.linkUrl(`/manager/role/edit/${scope.row.id}`)"></el-button>
...
<script setup>
	...
	const initData = async() => {
		let sendUrl = `/admin/manager/role?pageSize=${datas.pageSize}&pageIndex=${datas.pageIndex}`
	...
	const deleteCheckAll = () => {
		...
		proxy.$confirm('确认要删除该记录吗？', '提示', {
			...
			proxy.$api.request({
				method: 'delete',
				url: `/admin/manager/role?ids=${ids.toString()}`,
				loading: true,
				successMsg: '已删除成功',
				success(res) {
