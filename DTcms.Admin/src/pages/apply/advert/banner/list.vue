<template>
	<div class="mainbody">
		<dt-location :data="[{title:'应用管理'},{title:'广告管理'},{title:'广告内容'}]"></dt-location>
		<dt-toolbar-box>
			<template #default>
				<div class="list-box">
					<div class="l-list">
						<el-button-group>
							<el-button icon="Plus" @click="$common.linkUrl('/apply/advert/banner/edit')">新增</el-button>
			...
			<template #more>
				<div class="more-box">
					<dl>
						<dt>所属广告位</dt>
						<dd>
							<el-select v-model="datas.advertId" placeholder="请选择..." @change="initData">
		...
		<div class="content-box">
			<el-card class="table-card">
				<el-table ref="tableRef" v-loading="datas.loading" :data="datas.listData" stripe class="table-list" @selection-change="handleSelectionChange">
					...
					<el-table-column label="广告位" width="130" show-overflow-tooltip>
						<template #default="scope">
							<span>{{scope.row.advertTitle}}</span>
						</template>
					</el-table-column>
					<el-table-column prop="title" label="内容名称" min-width="180"></el-table-column>
					<el-table-column prop="startTime" label="开始时间" width="160" show-overflow-tooltip></el-table-column>
					<el-table-column prop="endTime" label="结束时间" width="160" show-overflow-tooltip></el-table-column>
					...
					<el-table-column fixed="right" label="操作" width="90">
						<template #default="scope">
							<el-button size="small" icon="Edit" @click="$common.linkUrl(`/apply/advert/banner/edit/${scope.row.id}`)"></el-button>
...
<script setup>
	...
	const initData = async() => {
		let sendUrl = `/admin/advert/banner?pageSize=${datas.pageSize}&pageIndex=${datas.pageIndex}`
		if (datas.advertId > 0) {
			sendUrl += `&parentId=${datas.advertId}`
		}
	...
	const deleteCheckAll = () => {
		...
		proxy.$confirm('确认要删除该记录吗？', '提示', {
			...
			proxy.$api.request({
				method: 'delete',
				url: `/admin/advert/banner?ids=${ids.toString()}`,
				loading: true,
				successMsg: '已删除成功',
				success(res) {
