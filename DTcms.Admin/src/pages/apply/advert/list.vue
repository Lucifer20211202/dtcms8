<template>
	<div class="mainbody">
		<dt-location :data="[{title:'应用管理'},{title:'广告位列表'}]"></dt-location>
		<dt-toolbar-box>
			<template #default>
				<div class="list-box">
					<div class="l-list">
						<el-button-group>
							<el-button icon="Plus" @click="$common.linkUrl('/apply/advert/edit')">新增</el-button>
		...
		<div class="content-box">
			<el-card class="table-card">
				<el-table ref="tableRef" v-loading="datas.loading" :data="datas.listData" stripe class="table-list" @selection-change="handleSelectionChange">
					...
					<el-table-column prop="callIndex" label="调用标识" min-width="120"></el-table-column>
					<el-table-column prop="title" label="广告位名称" min-width="180"></el-table-column>
					...
					<el-table-column fixed="right" label="操作" width="90">
						<template #default="scope">
							<el-button size="small" icon="Edit" @click="$common.linkUrl(`/apply/advert/edit/${scope.row.id}`)"></el-button>
...
<script setup>
	...
	const initData = async() => {
		let sendUrl = `/admin/advert?pageSize=${datas.pageSize}&pageIndex=${datas.pageIndex}`
	...
	const deleteCheckAll = () => {
		...
		proxy.$confirm('确认要删除该记录吗？', '提示', {
			...
			proxy.$api.request({
				method: 'delete',
				url: `/admin/advert?ids=${ids.toString()}`,
				loading: true,
				successMsg: '已删除成功',
				success(res) {
