		...
		<dt-toolbar-box>
			<template #default>
				<div class="list-box">
					<div class="l-list">
						<el-button-group>
							<el-button icon="Plus" @click="$common.linkUrl('/apply/feedback/edit')">新增</el-button>
		...
		<div class="content-box">
			<el-card class="table-card">
				<el-table ref="tableRef" v-loading="datas.loading" :data="datas.listData" stripe class="table-list" @selection-change="handleSelectionChange">
					...
					<el-table-column prop="content" label="留言内容" min-width="180">
						<template #default="scope">
							<span class="nowrap">{{scope.row.content}}</span>
						</template>
					</el-table-column>
					<el-table-column label="答复" width="80" align="center">
						<template #default="scope">
							<el-icon v-if="scope.row.replyTime"><Check /></el-icon>
							<el-icon v-else><Minus /></el-icon>
						</template>
					</el-table-column>
					<el-table-column prop="addTime" label="留言时间" width="160"></el-table-column>
					...
					<el-table-column fixed="right" label="操作" width="90">
						<template #default="scope">
							<el-button size="small" icon="Edit" @click="$common.linkUrl(`/apply/feedback/edit/${scope.row.id}`)"></el-button>
...
<script setup>
	...
	const initData = async() => {
		let sendUrl = `/admin/feedback?pageSize=${datas.pageSize}&pageIndex=${datas.pageIndex}`
	...
	const deleteCheckAll = () => {
		...
		proxy.$confirm('确认要删除该记录吗？', '提示', {
			...
			proxy.$api.request({
				method: 'delete',
				url: `/admin/feedback?ids=${ids.toString()}`,
				loading: true,
				successMsg: '已删除成功',
				success(res) {
