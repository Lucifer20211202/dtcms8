<template>
	<div class="mainbody">
		<dt-location :data="[{title:'系统功能'},{title:'前台菜单'}]"></dt-location>
		<dt-toolbar-box>
			<template #default>
				<div class="list-box">
					<div class="l-list">
						<el-button-group>
							<el-button icon="Plus" @click="$common.linkUrl('/setting/menu/edit')">新增</el-button>
			...
			<el-card class="table-card">
				<el-table ref="tableRef" v-loading="datas.loading" :data="datas.listData" row-key="id" default-expand-all stripe
					...
					<el-table-column fixed="right" label="操作" width="120">
						<template #default="scope">
							<el-button size="small" icon="Edit" @click="$common.linkUrl(`/setting/menu/edit/${scope.row.id}`)"></el-button>
							<el-button size="small" icon="Plus" @click="$common.linkUrl('/setting/menu/edit', {parentId:scope.row.id})"></el-button>
...
<script setup>
	...
	const deleteCheckAll = () => {
		...
		proxy.$confirm('确认要删除该记录吗？', '提示', {
			...
			proxy.$api.request({
				method: 'delete',
				url: `/admin/site/menu?ids=${ids.toString()}`,
				loading: true,
				successMsg: '已删除成功',
				success(res) {
