<template>
	<div class="mainbody">
		<dt-location :data="[{title:'系统功能'},{title:'支付方式'}]"></dt-location>
		<dt-toolbar-box>
			<template #default>
				<div class="list-box">
					<div class="l-list">
						<el-button-group>
							<el-button icon="Plus" @click="$common.linkUrl('/setting/payment/edit')">新增</el-button>
							<el-button icon="Delete" @click="deleteCheckAll">删除</el-button>
							<el-button icon="TakeawayBox" @click="$common.linkUrl('/setting/payee/list')">接入商</el-button>
						</el-button-group>
					</div>
					<div class="r-list">
		...
		<div class="content-box">
			<el-card class="table-card">
				<el-table ref="tableRef" v-loading="datas.loading" :data="datas.listData" stripe class="table-list" @selection-change="handleSelectionChange">
					...
					<el-table-column prop="payment.title" label="接入商" min-width="120"></el-table-column>
					<el-table-column prop="site.title" label="所属站点" min-width="120"></el-table-column>
					<el-table-column label="收款类型" width="120">
						<template #default="scope">
							<el-tag size="small" effect="dark" v-if="scope.row.payment.type==1">余额</el-tag>
							<el-tag size="small" type="danger" effect="dark" v-else-if="scope.row.payment.type==2">微信</el-tag>
							<el-tag size="small" type="success" effect="dark" v-else-if="scope.row.payment.type==3">支付宝</el-tag>
							<el-tag size="small" type="info" effect="dark" v-else>线下</el-tag>
						</template>
					</el-table-column>
					<el-table-column label="接口类型" width="120">
						<template #default="scope">
							<el-tag size="small" effect="dark" v-if="scope.row.type=='balance'">余额接口</el-tag>
							<el-tag size="small" type="danger" effect="dark" v-else-if="scope.row.type=='pc'">网页接口</el-tag>
							<el-tag size="small" effect="dark" v-else-if="scope.row.type=='native'">扫码接口</el-tag>
							<el-tag size="small" type="info" effect="dark" v-else>线下接口</el-tag>
						</template>
					</el-table-column>
					...
					<el-table-column fixed="right" label="操作" width="90">
						<template #default="scope">
							<el-button size="small" icon="Edit" @click="$common.linkUrl(`/setting/payment/edit/${scope.row.id}`)"></el-button>
...
<script setup>
	...
	const initData = async() => {
		let sendUrl = `/admin/site/payment?pageSize=${datas.pageSize}&pageIndex=${datas.pageIndex}`
	...
	const deleteCheckAll = () => {
		...
		proxy.$confirm('确认要删除该记录吗？', '提示', {
			...
			proxy.$api.request({
				method: 'delete',
				url: `/admin/site/payment?ids=${ids.toString()}`,
				loading: true,
				successMsg: '已删除成功',
				success(res) {
