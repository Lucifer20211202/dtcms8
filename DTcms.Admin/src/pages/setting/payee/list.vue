<template>
	<div class="mainbody">
		<dt-location :data="[{title:'系统功能'},{title:'支付平台'}]"></dt-location>
		<dt-toolbar-box>
			<template #default>
				<div class="list-box">
					<div class="l-list">
						<el-button-group>
							<el-button icon="Back" @click="$common.linkUrl('/setting/payment/list')">返回</el-button>
							<el-button icon="Plus" @click="$common.linkUrl('/setting/payee/edit')">新增</el-button>
		...
		<div class="content-box">
			<el-card class="table-card">
				<el-table ref="tableRef" v-loading="datas.loading" :data="datas.listData" stripe class="table-list" @selection-change="handleSelectionChange">
					...
					<el-table-column prop="title" label="平台名称" width="150"></el-table-column>
					<el-table-column label="收款类型" width="120">
						<template #default="scope">
							<el-tag size="small" effect="dark" v-if="scope.row.type==1">余额</el-tag>
							<el-tag size="small" type="danger" effect="dark" v-else-if="scope.row.type==2">微信</el-tag>
							<el-tag size="small" type="success" effect="dark" v-else-if="scope.row.type==3">支付宝</el-tag>
					...
					<el-table-column fixed="right" label="操作" width="90">
						<template #default="scope">
							<el-button size="small" icon="Edit" @click="$common.linkUrl(`/setting/payee/edit/${scope.row.id}`)"></el-button>
...
<script setup>
	...
	const initData = async() => {
		let sendUrl = `/admin/payment?pageSize=${datas.pageSize}&pageIndex=${datas.pageIndex}`
	...
	const deleteCheckAll = () => {
		...
		proxy.$confirm('确认要删除该记录吗？', '提示', {
			...
			proxy.$api.request({
				method: 'delete',
				url: `/admin/payment?ids=${ids.toString()}`,
				loading: true,
				successMsg: '已删除成功',
				success(res) {
