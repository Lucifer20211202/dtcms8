<template>
	<div class="mainbody">
		<dt-location :data="[{title:'系统功能'},{title:'授权绑定'}]"></dt-location>
		<dt-toolbar-box>
			<template #default>
				<div class="list-box">
					<div class="l-list">
						<el-button-group>
							<el-button icon="Back" @click="$common.linkUrl('/setting/oauth/list')">返回</el-button>
		...
		<div class="content-box">
			<el-card class="table-card">
				<el-table ref="tableRef" v-loading="datas.loading" :data="datas.listData" stripe class="table-list" @selection-change="handleSelectionChange">
					...
					<el-table-column prop="oAuthTitle" label="授权平台" width="150"></el-table-column>
					<el-table-column prop="userName" label="用户名" width="150"></el-table-column>
					<el-table-column prop="openId" label="绑定值" min-width="120"></el-table-column>
					<el-table-column prop="addTime" label="绑定时间" width="160"></el-table-column>
...
<script setup>
	...
	const initData = async() => {
		let sendUrl = `/admin/site/oauth/login?pageSize=${datas.pageSize}&pageIndex=${datas.pageIndex}`
	...
	const deleteCheckAll = () => {
		...
		proxy.$confirm('确认要删除该记录吗？', '提示', {
			...
			proxy.$api.request({
				method: 'delete',
				url: `/admin/site/oauth/login?ids=${ids.toString()}`,
				loading: true,
				successMsg: '已删除成功',
				success(res) {
	...
	const deleteItem = (val) => {
		...
		proxy.$confirm('确认要删除该记录吗？', '提示', {
			...
			proxy.$api.request({
				method: 'delete',
				url: `/admin/site/oauth/login/${val}`,
				loading: true,
				successMsg: '已删除成功',
				success(res) {
					initData() //重新加载列表
