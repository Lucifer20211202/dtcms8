<template>
	<div class="mainbody">
		<dt-location :data="[{title:'系统功能'},{title:'授权平台'}]"></dt-location>
		<dt-toolbar-box>
			<template #default>
				<div class="list-box">
					<div class="l-list">
						<el-button-group>
							<el-button icon="Plus" @click="$common.linkUrl('/setting/oauth/edit')">新增</el-button>
							<el-button icon="Delete" @click="deleteCheckAll">删除</el-button>
							<el-button icon="User" @click="$common.linkUrl('/setting/oauth/login')">授权记录</el-button>
						</el-button-group>
					</div>
					<div class="r-list">
		...
		<div class="content-box">
			<el-card class="table-card">
				<el-table ref="tableRef" v-loading="datas.loading" :data="datas.listData" stripe class="table-list" @selection-change="handleSelectionChange">
					...
					<el-table-column prop="title" label="应用标题" min-width="120"></el-table-column>
					<el-table-column label="应用类型" width="120">
						<template #default="scope">
							<el-tag size="small" effect="dark" v-if="scope.row.type=='web'">网页</el-tag>
							<el-tag size="small" type="info" effect="dark" v-else>未知</el-tag>
						</template>
					</el-table-column>
					<el-table-column label="所属站点" width="150">
						<template #default="scope">
							<span class="nowrap">{{scope.row.site.title}}</span>
						</template>
					</el-table-column>
					...
					<el-table-column fixed="right" label="操作" width="90">
						<template #default="scope">
							<el-button size="small" icon="Edit" @click="$common.linkUrl(`/setting/oauth/edit/${scope.row.id}`)"></el-button>
...
<script setup>
	...
	const initData = async() => {
		let sendUrl = `/admin/site/oauth?pageSize=${datas.pageSize}&pageIndex=${datas.pageIndex}`
	...
	const deleteCheckAll = () => {
		...
		proxy.$confirm('确认要删除该记录吗？', '提示', {
			...
			proxy.$api.request({
				method: 'delete',
				url: `/admin/site/oauth?ids=${ids.toString()}`,
				loading: true,
				successMsg: '已删除成功',
				success(res) {
