		...
		<el-skeleton :loading="datas.loading" style="height:100%;" animated>
			...
			<template #default>
				<el-carousel :interval="props.interval" height="100%" style="height:100%;">
					<el-carousel-item v-for="(item,index) in datas.listData.banners" :key="index" style="height:100%;">
						<a :href="item.linkUrl" class="list-box">
...
<script setup>
	...
	//获取广告位信息（服务端获取方式）
	/*const {data:advertData} = await useAsyncData('advertbanner', () => useHttp({url: `/client/advert/${props.index}?siteId=${siteConfig.id}`}))
