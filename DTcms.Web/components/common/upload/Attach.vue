		...
		<div class="list-wrap">
			<div class="list-box" v-for="(item,index) in datas.listData" :key="index"
				...
				<div class="img-box">
					...
					<el-icon v-else><ElIconLoading /></el-icon>
				</div>
				<div class="info-box">
					<h3>{{item.fileName}}</h3>
					<dl>
						...
						<dd>
							<span @click="handleUpdate(item,index)"><el-icon><ElIconEdit /></el-icon></span>
						</dd>
					</dl>
				</div>
				<div class="close" @click="handleRemove(item)"><el-icon><ElIconClose /></el-icon></div>
...
<script setup>
	...
	const handleSuccess = (data, item) => {
		item.url = common.loadFile(data[0].filePath)
